...
fn shards(path: &str) -> Vec<PathBuf> {
    fs::read_dir(path)
        .unwrap()
        .map(|x| x.unwrap().path())
        .filter(|x| x.extension().unwrap().to_str() == Some("txt"))
        .collect()
}
...
fn shards_latest_timestamp(path: &str) -> SystemTime {
    shards(path)
        .iter()
        .map(|x| fs::metadata(x).unwrap().modified().unwrap())
        .max()
        .unwrap()
}
...
pub fn parse_from_txt(path: String) -> DictionaryOfStringArticles {
    ...
    shards.sort();
    println!(
        "Loading data for dictionary {}, from {} text shards",
        path,
        shards.len()
    );
...
fn build_index(articles: &Vec<String>) -> BTreeMap<String, Vec<usize>> {
    ...
    for (key, weighted_entry_indices) in index_with_weights.iter_mut() {
        ...
        compact_index.insert(
            key.to_lowercase(),
            weighted_entry_indices.iter().map(|x| x.0).collect(),
        );
    }
