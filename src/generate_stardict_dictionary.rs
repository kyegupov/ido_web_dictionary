...
fn main() {
    ...
    for &dir in DIRECTIONS.iter() {
        ...
        let mut dict_file_stream = File::create(target_path.to_owned() + "/" + dir + ".dict").unwrap();
        ...
        let mut idx_file_stream = File::create(target_path.to_owned() + "/" + dir + ".idx").unwrap();
        let mut all_words: Vec<String> = data.compact_index.keys().map(|x|x.clone()).collect();
        // Emulate g_ascii_strcasecmp
        all_words.sort_by_key(|x| x.bytes().map(|x| if x>=128 { 0 } else {x}).collect::<Vec<u8>>());
        let 
        mut idx_size = 0;
        for word in &all_words {
            ...
            for article_id in article_ids {
                ...
                idx_file_stream.write_i32::<BigEndian>(idx_data[*article_id].0 as i32).unwrap();
                idx_file_stream.write_i32::<BigEndian>(idx_data[*article_id].1 as i32).unwrap();
                idx_size += word_bytes.len() + 9;
            }
        }
        let mut ifo_file_writer = File::create(target_path.to_owned() + "/" + dir + ".ifo").unwrap();
        ifo_file_writer.write_all(format!("StarDict's dict ifo file
version=3.0.0
[options]
bookname={}
wordcount={}
idxfilesize={}
sametypesequence=h
", dir, all_words.len(), idx_size).as_bytes()).unwrap();
    }
}
